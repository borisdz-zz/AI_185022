...
class Molecule(Problem):
    ...
    def h(self, node):
        # Хевристичка функција која за секоја состојба пресметува минимален број на турнувањ на паровите
        # од атоми за тие да се спојат
        state = node.state
        h1 = state[0], state[1]
        o = state[2], state[3]
        h2 = state[4], state[5]
        ...
        # проверка за позициите на Н1 и О и потребниот број на чекори за да се спојат
        if h1[0] != o[0]:
            if h1[1] != (o[1] - 1):
                # ако атомот на водород не е во иста редица и колона веднаш до атомот
                # на кислород ни требаат најмалку 2 турнувања (turn down/up + left/right) за да се спојат
                value += 2
            else:
                # ако атомот на водород не е во иста редица, но во колона веднаш до атомот на кислорот
                # ни треба најмалку 1 турнување (turn up or down) за да се спојат
                value += 1
        else:  # h1[0] == o[0]
            if h1[1] > o[1]:
                # ако атомот на водород е во иста редица, но во колона од десна страна на атомот на кислород
                # ни требаат најмалку 3 турнувања (turn 2 x down/up + left/right) за да се спојат
                value += 3
            elif h1[1] < (o[1] - 1):
                # ако атомот на водород е во иста редица, но во колона од лева страна на атомот на кислород
                # ни треба најмалку 1 турнување (turn right) за да се спојат
                value += 1

        # проверка за позициите на Н2 и О и потребниот број на чекори за да се спојат
